...
{
  return 2*(urg->parameters_.area_max_ + 1); //2x the space for intensity data...
}
                    ...
                    int last_index)
{
  ...
  if ((request_type == URG_GD_INTENSITY) ||
    ...
    if (! strcmp("UTM-30LX", urg->parameters_.sensor_type)) {
      ...
      //ABE: Don't see any reason why this should be hardcoded...
      //      /* cluster value */
      //      buffer[10] = '0';
      //      buffer[11] = '2';
                                 ...
                                 long intensity[])
{
  int i;
  int n;
  int skip_lines = urg->skip_lines_;
  int data_size = internal_receiveData(urg, data, data_max,
                           0, data_max, skip_lines);

  n = data_size/skip_lines/2;

  int filled =0;
  for (i = 0; i < n; i++) {
    int index = 2*skip_lines*i;
    long length = data[index];
    long intensity_value = data[index + skip_lines];
    for (int j=0;j < skip_lines; j++){
      data[filled] = length;
      intensity[filled] = intensity_value;
      filled++;
    }
  }
  return filled;
