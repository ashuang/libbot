...
struct moxa
{
    ...
    bot_ssocket_t *cmd_socket;
};

static int moxa_serial_close(void *ptr)
{
    struct moxa *moxa = (struct moxa*) ptr;
  
    if (moxa->ipaddr)
        free(moxa->ipaddr);
    if (moxa->data_socket)
        bot_ssocket_destroy(moxa->data_socket);
    if (moxa->cmd_socket)
        bot_ssocket_destroy(moxa->cmd_socket);

    free(moxa);
    return 0;
}

static void *moxa_serial_open(const char *ipaddr, int physport)
{
    int res;
    struct moxa *moxa;

    moxa = (struct moxa*) calloc(1, sizeof(struct moxa));
    ...
    moxa->data_socket = bot_ssocket_create();
    res = bot_ssocket_connect(moxa->data_socket, ipaddr, 4001 + physport);
    if (res) 
        goto error;

    moxa->cmd_socket = bot_ssocket_create();
    res = bot_ssocket_connect(moxa->cmd_socket, ipaddr, 966 + physport);
    if (res)
        goto error;
...
static void *moxa_serial_open_url(const char *url)
{
...
//    printf("ipaddr: %s, port: %s\n", ipaddr, port);
    void *res = moxa_serial_open(ipaddr, atoi(port));
    free(tmp);

    return res;
}

static int moxa_serial_getfd(void *ptr)
{
    struct moxa *moxa = (struct moxa*) ptr;

    return bot_ssocket_get_fd(moxa->data_socket);
}

static int moxa_wait_ack(struct moxa *moxa, char cmd)
{
    fd_set  readfds;
    char    buf[20];
    ...
    timeout.tv_sec = 2;
    FD_ZERO(&readfds);
    int fd = bot_ssocket_get_fd(moxa->cmd_socket);
...
static int moxa_send_command(struct moxa *moxa, void *cmd, int cmdlen)
{
    int tries = 0;

    while (1) {
        int res = send(bot_ssocket_get_fd(moxa->cmd_socket), cmd, 6, 0);
        if (res != 6) 
            perror("send");
...
static int moxa_serial_setbaud(void *ptr, int baud)
{
    struct moxa *moxa = (struct moxa*) ptr;
...
struct sick_serial_ops moxa_serial_ops = {
    .sick_serial_open = moxa_serial_open_url,
    .sick_serial_getfd = moxa_serial_getfd,
    .sick_serial_setbaud = moxa_serial_setbaud,
    .sick_serial_close = moxa_serial_close,
};
