...
static void on_frame_update(const lcm_recv_buf_t *rbuf, const char *channel, const bot_core_isometry_t *msg,
    void *user_data)
{
  BotCoordFrames * bot_coord_frames = (BotCoordFrames *) user_data;
  g_mutex_lock(bot_coord_frames->mutex);
  BotTrans link_transf;
  bot_trans_set_from_quat_trans(&link_transf, msg->quat, msg->trans);

  frame_handle_t * frame_handle = (frame_handle_t *) g_hash_table_lookup(bot_coord_frames->frame_handles_by_channel,
      channel);
  assert(frame_handle != NULL);
  frame_handle->was_updated = 1;
  bot_ctrans_link_update(frame_handle->ctrans_link, &link_transf, msg->utime);
  g_mutex_unlock(bot_coord_frames->mutex);

}

BotCoordFrames *
bot_coord_frames_new(lcm_t *lcm, BotParam *config)
{
  BotCoordFrames *self = g_slice_new0(BotCoordFrames);
  ...
  if (num_frames <= 0) {
    fprintf(stderr, "BotCoordFrames Error: param file does not contain a 'coordinate_frames' block\n");
    bot_coord_frames_destroy(self);
    return NULL;
  }
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    if (ret < 0) {
      fprintf(stderr, "BotCoordFrames Error: frame %s does not have a 'relative_to' field block\n", frame_name);
      goto fail;
    }
    ...
    if (ret < 0) {
      fprintf(stderr, "BotCoordFrames Error: could not get history size for frame %s\n", frame_name);
      goto fail;
    }
    ...
    if (bot_param_get_num_subkeys(self->config, param_key) != 2) {
      fprintf(stderr,
          "BotCoordFrames Error: frame %s does not have the right number of fields in the 'initial_transform' block\n",
          frame_name);
      goto fail;
    }
    BotTrans init_trans;
    ...
    if (ret < 0) {
      fprintf(stderr, "BotCoordFrames Error: could not get 'initial_transform' for frame %s\n", frame_name);
      goto fail;
    }
    ...
    if (frame_handle != NULL) {
      fprintf(stderr, "BotCoordFrames Error: frame %s duplicated\n", frame_name);
      goto fail;
    }
    ...
    if (history > 0) {
      ...
      if (ret < 0) {
        fprintf(stderr, "BotCoordFrames Error: could not get 'update_channel' for frame %s\n", frame_name);
        goto fail;
      }
      ...
      if (entry != NULL) {
        fprintf(stderr, "BotCoordFrames Error: update_channel %s for frame %s already used for frame %s\n",
...
void bot_coord_frames_destroy(BotCoordFrames * bot_coord_frames)
{
  ...
  g_mutex_free(bot_coord_frames->mutex);

  bot_ctrans_destroy(bot_coord_frames->ctrans);
  g_slice_free(BotCoordFrames, bot_coord_frames);
}
...
{
  g_mutex_lock(bot_coord_frames->mutex);
  int status = bot_ctrans_get_trans(bot_coord_frames->ctrans, from_frame, to_frame, utime, result);
  g_mutex_unlock(bot_coord_frames->mutex);
  return status;
}

int bot_coord_frames_get_trans(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    BotTrans *result)
{
  g_mutex_lock(bot_coord_frames->mutex);
  int status = bot_ctrans_get_trans_latest(bot_coord_frames->ctrans, from_frame, to_frame, result);
  g_mutex_unlock(bot_coord_frames->mutex);
  return status;
}

int bot_coord_frames_get_trans_mat_3x4(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    double mat[12])
{
  BotTrans bt;
  if (!bot_coord_frames_get_trans(bot_coord_frames, from_frame, to_frame, &bt))
    return 0;
  bot_trans_get_mat_3x4(&bt, mat);
  return 1;
}

int bot_coord_frames_get_trans_mat_3x4_with_utime(BotCoordFrames *bot_coord_frames, const char *from_frame,
    const char *to_frame, int64_t utime, double mat[12])
{
  BotTrans bt;
  if (!bot_coord_frames_get_trans_with_utime(bot_coord_frames, from_frame, to_frame, utime, &bt))
    return 0;
  bot_trans_get_mat_3x4(&bt, mat);
  return 1;
}

int bot_coord_frames_get_trans_mat_4x4(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    double mat[12])
{
  BotTrans bt;
  if (!bot_coord_frames_get_trans(bot_coord_frames, from_frame, to_frame, &bt))
...
int bot_coord_frames_get_trans_mat_4x4_with_utime(BotCoordFrames *bot_coord_frames, const char *from_frame,
    const char *to_frame, int64_t utime, double mat[12])
{
  BotTrans bt;
  if (!bot_coord_frames_get_trans_with_utime(bot_coord_frames, from_frame, to_frame, utime, &bt))
...
int bot_coord_frames_get_trans_latest_timestamp(BotCoordFrames *bot_coord_frames, const char *from_frame,
    const char *to_frame, int64_t *timestamp)
{
  g_mutex_lock(bot_coord_frames->mutex);
  int status = bot_ctrans_get_trans_latest_timestamp(bot_coord_frames->ctrans, from_frame, to_frame, timestamp);
  g_mutex_unlock(bot_coord_frames->mutex);
  return status;
}

int bot_coord_frames_have_trans(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame)
{
  g_mutex_lock(bot_coord_frames->mutex);
  int status = bot_ctrans_have_trans(bot_coord_frames->ctrans, from_frame, to_frame);
  g_mutex_unlock(bot_coord_frames->mutex);
  return status;
}

int bot_coord_frames_transform_vec(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    const double src[3], double dst[3])
{
  BotTrans rbtrans;
  if (!bot_coord_frames_get_trans(bot_coord_frames, from_frame, to_frame, &rbtrans))
    return 0;
  bot_trans_apply_vec(&rbtrans, src, dst);
  return 1;
}

int bot_coord_frames_rotate_vec(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    const double src[3], double dst[3])
{
  BotTrans rbtrans;
  if (!bot_coord_frames_get_trans(bot_coord_frames, from_frame, to_frame, &rbtrans))
...
int bot_coord_frames_get_n_trans(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
    int nth_from_latest)
{
  BotCTransLink *link = bot_ctrans_get_link(bot_coord_frames->ctrans, from_frame, to_frame);
  if (!link)
    return 0;
 ...
 */
int bot_coord_frames_get_nth_trans(BotCoordFrames *bot_coord_frames, const char *from_frame, const char *to_frame,
...
static BotCoordFrames *global_bot_coord_frames = NULL;
static GStaticMutex bot_coord_frames_global_mutex = G_STATIC_MUTEX_INIT;

BotCoordFrames*
bot_coord_frames_get_global(lcm_t *lcm, BotParam *config)
{
  if (lcm == NULL)
  ...
  g_static_mutex_lock(&bot_coord_frames_global_mutex);
  if (global_bot_coord_frames == NULL) {
    global_bot_coord_frames = bot_coord_frames_new(lcm, config);
    if (!global_bot_coord_frames)
      goto fail;
  }

  BotCoordFrames *result = global_bot_coord_frames;
  g_static_mutex_unlock(&bot_coord_frames_global_mutex);
  return result;

  fail: g_static_mutex_unlock(&bot_coord_frames_global_mutex);
  fprintf(stderr, "ERROR: Could not get global bot_coord_frames!\n");
  return NULL;
}
